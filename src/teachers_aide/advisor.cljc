(ns teachers-aide.advisor
  "AideAdvisor — proposes a teacher's-aide operation (assist step,
  monitor entry, direct one-on-one supervision of a minor without a
  lead teacher present, behavioral-incident response) for a
  registered classroom. The advisor is swappable: `mock-advisor`
  (deterministic, default in dev/tests/CI) or `llm-advisor` (wraps a
  real `langchain.model/ChatModel`). Either way the advisor ONLY
  produces a PROPOSAL — it never writes to the store and has no
  notion of classroom provenance or supervision/incident risk;
  `teachers-aide.governor` is the independent system that decides
  whether the proposal may proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :assist|:monitor|:one-on-one-supervision-without-lead-teacher|:behavioral-incident-response
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for classroom " (:classroom-id request))})
...
(def ^:private system-prompt
  "You are a teacher's-aide advisor. Given an assist or monitor
