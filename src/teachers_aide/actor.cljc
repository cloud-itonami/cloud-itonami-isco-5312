(ns teachers-aide.actor
  "TeachersAideActor — the ISCO-08 5312 independent teacher's-aide
  actor as a `langgraph.graph/state-graph` (per ADR-2607011000 /
  CLAUDE.md Actors section). One graph run = one assist/monitor
  ...
  The unconditional invariant: the AideAdvisor can never directly
  commit a record or dispatch a robot action the TeachersAideGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [teachers-aide.advisor :as advisor]
            [teachers-aide.governor :as governor]
            [teachers-aide.store :as store]))

(defn build-graph
  "Build a compiled TeachersAideActor graph. `store` implements
  `teachers-aide.store/Store`. `advisor` implements
  `teachers-aide.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:classroom-id (:classroom-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
